...
void send_t(const problem &problem, int id_worker, int id_w_target) {
    ...
    for(uint64_t r=0; r<size-1; r++){
        ...
        MPI_Send(&problem.M[rel_index], size-r-1, MPI_DOUBLE, id_w_target, 0, MPI_COMM_WORLD);
    }
}
...
void recv_t(problem &problem, int id_worker, int id_w_target) {
    ...
    for(uint64_t r=0; r<size-1; r++){
        auto rel_index = abs_to_rel_index(problem, id_worker, std::pair(index+r, index+r+1));
        MPI_Recv(&problem.M[rel_index], size-r-1, MPI_DOUBLE, id_w_target, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
    }
}
