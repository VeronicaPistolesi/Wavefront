...
void send_t(const problem &problem, int id_worker, int id_w_sender, std::vector<MPI_Request> &requests, std::vector<MPI_Comm> &communicators) {
    ...
    if(id_worker==id_w_sender){
        ...
        for(uint64_t r=0; r<size-1; r++){
            ...
            for(int w=0; w<id_w_sender; w++){
                ...
                //std::cout << id_w_sender << " " << w << " " << send_count[w] << " " << displacement[w] << std::endl;
            }
            requests.push_back(MPI_Request());
            //std::cout << send_count[id_w_sender] << " " << displacement[id_w_sender] << std::endl;
            MPI_Iscatterv(problem.M.data(), send_count.data(), displacement.data(), MPI_DOUBLE, nullptr, 0, MPI_DOUBLE, id_w_sender, communicators[id_w_sender], &requests.back());
        }
    //receive
    } else {
        for(uint64_t r=0; r<size-1; r++){
            auto rel_index = abs_to_rel_index(problem, id_worker, std::pair(index+r, index+r+1));
            requests.push_back(MPI_Request());
            //std::cout << id_w_sender << " " << id_worker << std::endl;
            MPI_Iscatterv(nullptr, nullptr, nullptr, MPI_DOUBLE, &problem.M[rel_index], size-r-1, MPI_DOUBLE, id_w_sender, communicators[id_w_sender], &requests.back());
        }
    }    
}


void send_r(const problem &problem, int id_worker, uint64_t iter, int id_w_sender, std::vector<MPI_Request> &requests, std::vector<MPI_Comm> &communicators) {
    ...
    //receive
        ...
        for(uint64_t r=index.first; r<index.first+size.first; r++){
            auto rel_index = abs_to_rel_index(problem, id_worker, std::pair(r, index.second));
            requests.push_back(MPI_Request());
            MPI_Iscatterv(nullptr, nullptr, nullptr, MPI_DOUBLE, &problem.M[rel_index], size.second, MPI_DOUBLE, id_w_sender, communicators[id_w_sender], &requests.back());
        }
    }
}    
...
int main (int argc, char *argv[]) {
    ...
    // vettore requests
    auto scatterv_size_t = sum_height(problem, id_worker+1)-1*(n_worker-id_worker-1);
    std::vector<MPI_Request> requests_t;
    //std::cout << sum_height(problem, id_worker+1) << " " << -1*(n_worker-id_worker) << std::endl;
    requests_t.reserve(scatterv_size_t);

    for(int id_w_sender = std::max(1, id_worker); id_w_sender<n_worker; id_w_sender++){
        send_t(problem, id_worker, id_w_sender, requests_t, communicators);
    }
    ...
    for(uint64_t iter=0; iter<n_worker-id_worker-1; iter++){
        ...
        // vettore requests
        auto scatterv_size_r = sum_height(problem, id_worker+1)-sum_height(problem, n_worker-iter-1);
        std::vector<MPI_Request> requests_r;
        requests_r.reserve(scatterv_size_r);

        for(int id_w_sender = std::max(1, id_worker); id_w_sender<n_worker-iter-1; id_w_sender++){
            send_r(problem, id_worker, iter, id_w_sender, requests_r, communicators);
        }
