...
csv_headers = ['file_name', 'N', 'n_workers', 'threads'] + [f'time{i}' for i in range(1)] + ['mean_time']

for file_name in os.listdir(folder_path):
    executable = os.path.join(folder_path, file_name.replace('.cpp', ''))
    
    if 'mpi_w0_async' in file_name and 'll' not in file_name:
        for params in param_combinations_mpi:
            times = []
            for _ in range(1):  
                command = ['mpiexec', '-n', str(params[0]), '--bind-to', 'none', executable, str(params[1])]
...
csv_file = 'res/tests_mpi_w0_async_d.csv'
